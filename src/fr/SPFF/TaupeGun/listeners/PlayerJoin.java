...
class PlayerJoin {
    ...
    void handle(final PlayerJoinEvent e){
        if(!this.listening.getMain().getTaupeGunManager().getState().equals(TaupeGunManager.State.WAITING)){
            PlayerTaupe playerTaupe = null;
            for (final PlayerTaupe playerTaupes : PlayerTaupe.getPlayerTaupeList()) {
                if (playerTaupes.getPlayer().getName().equalsIgnoreCase(e.getPlayer().getName()))
                    playerTaupe = playerTaupes;
            }
            if (this.listening.getMain().getTaupeGunManager().getTimer() < this.listening.getMain().getTaupeGunManager().taupeTime && playerTaupe != null) {
